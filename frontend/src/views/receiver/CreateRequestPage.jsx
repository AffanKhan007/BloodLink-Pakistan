    ...
    try {
      ...
      if (form.urgency_level === "critical") {
        storeNavigationState(
          `/blood-radar?blood_group=${request.blood_group_needed}&city=${encodeURIComponent(request.city)}`,
          {
            flashSuccess: "Critical request created. Blood Radar is scanning for available blood now.",
            createdRequest: request,
          }
        );
        router.push(`/blood-radar?blood_group=${request.blood_group_needed}&city=${encodeURIComponent(request.city)}`);
      } else {
        storeNavigationState(`/receiver/requests/${request.id}`, {
          flashSuccess: "Request created, hospital slip uploaded, and compatible donors have been checked automatically.",
          createdRequest: request,
        });
        router.push(`/receiver/requests/${request.id}`);
      }
    } catch (submitError) {
