...
export default function BloodRadarPage() {
  ...
  useEffect(() => {
    ...
    import("../../components/leafletIcons")
      .then((mod) => {
        if (mounted) setLeafletIcons(mod);
      })
      .catch(() => {});
    return () => {
      mounted = false;
  ...
  const mapMarkers = useMemo(() => {
    ...
    banks.forEach((b, i) => {
      if (b.latitude && b.longitude) {
        markers.push({ key: `bank-${b.id}`, pos: [b.latitude, b.longitude], icon: leafletIcons.bankIcon(), popup: b.name });
      }
    });
    donors.forEach((d, i) => {
    ...
    drives.forEach((dr, i) => {
      if (dr.latitude && dr.longitude) {
        markers.push({ key: `drive-${dr.id}`, pos: [dr.latitude, dr.longitude], icon: leafletIcons.driveIcon(), popup: dr.title });
      }
    });
    return markers;
  }, [banks, donors, drives, leafletIcons]);
                  ...
                  {banks.map((b) => (
                    <div className="info-card" key={b.id}>
                      ...
                      <div className="radar-result-actions">
                        <Link className="button button-secondary button-full" href={`/blood-banks/${b.id}`}>
                          View full profile
                        </Link>
                      </div>
                    </div>
                ...
                <div className="stacked-cards">
                  {donors.map((d) => (
                    <div className="info-card" key={d.anon_id}>
                      ...
                      <div className="radar-result-actions">
                        <Link className="button button-primary button-full" href="/receiver/create-request">
                          Send match request
                        </Link>
                      </div>
                    </div>
                ...
                <div className="stacked-cards">
                  {drives.map((dr) => (
                    <div className="info-card" key={dr.id}>
                      ...
                      <div className="radar-result-actions">
                        <Link className="button button-secondary button-full" href={`/drives/${dr.id}`}>
                          View details
                        </Link>
                      </div>
                    </div>
